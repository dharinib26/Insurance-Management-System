...
public class JwtUtil {
	...
	public void validateToken(final String token) {
		Jwts.parserBuilder().setSigningKey(getSignKey()).build().parseClaimsJws(token);// Validates the JWT token
	}
	...
	private Key getSignKey() {
		...
		return Keys.hmacShaKeyFor(keyBytes);// Generates the signing key from the secret
	}
